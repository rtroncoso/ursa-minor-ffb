...
use crate::preset::{SimVarLayout, CORE_SIMVAR_COUNT};
...
pub fn parse_main_elems(
    ...
    let mut fv = FlightVars {
        airspeed_indicated: elem.first().copied().unwrap_or(0.0),
    ...
    for (i, key) in layout.extra_keys.iter().enumerate() {
        let idx = CORE_SIMVAR_COUNT + i;
        if let Some(v) = elem.get(idx).copied() {
...
mod tests {
    ...
    fn pause_from_events_or_elem() {
        ...
        paused_elem[10] = 1.0;
        let from_var = parse_main_elems(&paused_elem, &layout, false, 1.0);
    ...
    fn extra_simvar_populates_extras_map() {
        let layout = SimVarLayout {
            extra_keys: vec!["spoilers_pct".to_string()],
        };
